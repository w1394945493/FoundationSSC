...
class CreateDepthFromLiDAR(object):
    def __init__(self, data_root=None, dataset="kitti", load_seg=False):
        ...
        if self.dataset == "kitti":
            ...
            if self.load_seg:
                label_filename = os.path.join(
                    ...
                    seq_id,
                    "labels",
                    filename.replace(".png", ".label"),
                )
                label_points = np.fromfile(label_filename, dtype=np.uint32).reshape(
                    -1, 1, 1
                )
